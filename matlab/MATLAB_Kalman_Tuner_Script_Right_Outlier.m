function matlab_right_claw_tuner()
% MATLAB Interactive Script for Manual Kalman Filter Tuning of the RIGHT CLAW
...
% --- 2. Extract Right Claw Data ---
time_vector = data_table.Time;
% FSRs 5-8 are the right claw (columns 8 to 11 in the CSV)
fsr_signals_right = table2array(data_table(:, 8:11)); 

% --- 3. Create the Interactive Figure ---
fig = figure('Name', 'Interactive Kalman Tuner - RIGHT CLAW', 'NumberTitle', 'off', 'WindowState', 'maximized');
ax = axes('Parent', fig, 'Position', [0.1 0.35 0.85 0.55]);
hold(ax, 'on');
grid(ax, 'on');
title(ax, 'Right Claw Kalman Filter Performance');
xlabel(ax, 'Time (s)');
...
colors = lines(4);
for i = 1:4
    p_fsr(i) = plot(ax, time_vector, fsr_signals_right(:, i), ':', 'LineWidth', 1.5, 'Color', colors(i,:), 'DisplayName', ['FSR ' num2str(i+4)]);
end

% Plot filtered output
p_filtered = plot(ax, time_vector, zeros(size(time_vector)), 'r-', 'LineWidth', 2.0, 'DisplayName', 'Kalman Filtered Output');
...
q_label = uicontrol('Parent', fig, 'Style', 'text', 'Units', 'normalized', ...
    'Position', [0.1 0.23 0.8 0.03], 'String', 'Process Noise (Q_right)');
...
for i = 1:4
    ...
    r_labels(i) = uicontrol('Parent', fig, 'Style', 'text', 'Units', 'normalized', ...
        'Position', [0.1 + (i-1)*0.2, slider_y_pos+0.03, 0.18, 0.03], 'String', ['R_right(' num2str(i+4) ')']);
end

% Checkboxes to enable/disable sensors
...
y_pos = 0.86;
for i = 1:4
    sensor_checkboxes(i) = uicontrol('Parent', fig, 'Style', 'checkbox', 'String', ['FSR ' num2str(i+4)], ...
...
% --- Helper Functions ---
    function update_plot()
        ...
        for i = 1:4
            ...
            r_labels(i).String = ['R_right(' num2str(i+4) '): ' num2str(R_diag_full(i), '%.e')];
        end
        q_label.String = ['Process Noise (Q_right): ', num2str(Q_val, '%.e')];
        ...
        active_data = fsr_signals_right(:, active_sensors_idx);
        ...
        py_code_str = sprintf('Q_right = np.array([[%.e]])\nR_right = np.diag([%.4f, %.4f, %.4f, %.4f])', ...
